    ...
    <div className="min-h-screen relative">
      ...
      <section className="py-20 bg-gradient-to-br from-blue-50 to-purple-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">Automation Tools</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Powerful automation tools to streamline your development workflow and boost productivity.
          </p>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          {automationTools.map((tool, index) => (
            <Card 
              key={index} 
              className={`hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 ${expandedTool === index ? 'ring-2 ring-indigo-500' : ''}`}
            >
              <CardContent className="p-6">
                {/* Tool header */}
                <div className="flex items-center space-x-4 mb-6">
                  <div className={`text-4xl w-16 h-16 flex items-center justify-center bg-gradient-to-br ${tool.color} rounded-xl text-white shadow-lg`}>
                    {tool.icon}
                  </div>
                  <div>
